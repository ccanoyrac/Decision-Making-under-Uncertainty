...
# ── CFA weights (trained by CFA_Training.py → Data/CFA_weights.csv) ──────────
...
_VFA_PATH = os.path.join(_dir, 'Data', 'CFA_weights.csv')
try:
    _VFA = {}
    with open(_VFA_PATH, mode='r', newline='', encoding='utf-8') as _f:
        for row in csv.DictReader(_f):
            t = int(row['t'])
            _VFA[t] = {k: float(v) for k, v in row.items() if k != 't'}

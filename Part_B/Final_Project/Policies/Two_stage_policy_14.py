"""
Two-stage Policy — Group 14, DTU 02435, Spring 2026.
...
def _solve_sp(state, nodes):
    ...
    for nd in nodes:
        ...
        # Use PARENT's post-action temperature (= temperature entering this node,
        # before this node's action). Matches the environment: y_low is cleared
        # when T_n >= T_OK from the PREVIOUS step — not from the current action.
        # At root (pid is None) y_lo1_0 is already set consistently by the env.
        if pid is not None:
